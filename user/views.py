...
def user_login(request):
    if request.POST:
        ...
        if user is not None and user.is_active:
            ...
            return render(request, 'base.html', {'message': 'Username and/or password do not match!'})
    else:
...
def user_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_user = form.save()
            return render(request, 'base.html', {'message': 'Congratulations! Your account has been successfully registered. Please login.'})
        else:
